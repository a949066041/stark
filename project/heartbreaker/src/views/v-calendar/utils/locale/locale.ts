...
const monthUpdate = (arrName) => (d, v, l) => {
  const index = l[arrName].indexOf(v.charAt(0).toUpperCase() + v.substring(1).toLowerCase());
  if (~index) {
    d.month = index;
  }
};
...
export class Locale {
  ...
  format(date, mask) {
    ...
    mask = mask.replace(token, ($0) =>
      $0 in formatFlags ? formatFlags[$0](dateParts, this) : $0.slice(1, $0.length - 1)
    );
  ...
  normalizeMasks(masks) {
    return ((arrayHasItems(masks) && masks) || [(isString(masks) && masks) || "YYYY-MM-DD"]).map((m) =>
      maskMacros.reduce((prev, curr) => prev.replace(curr, this.masks[curr] || ""), m)
    );
  }
  ...
  getDateFromParts(parts) {
    ...
    if (this.timezone) {
      const dateString = `${pad(year, 4)}-${pad(month, 2)}-${pad(day, 2)}T${pad(hrs, 2)}:${pad(min, 2)}:${pad(
        sec,
        2
      )}.${pad(ms, 3)}`;
  ...
  getTimezoneOffset(parts) {
    ...
    if (this.timezone) {
      const dateString = `${pad(y, 4)}-${pad(m, 2)}-${pad(d, 2)}T${pad(hrs, 2)}:${pad(min, 2)}:${pad(sec, 2)}.${pad(
        ms,
        3
      )}`;
